        ...
        for j in targetWordIndices:
            ...
            for k in xrange(len(sourceNeighborhood[0])):
                for l in xrange(len(targetNeighborhood[0])):
                      if (sourceNeighborhood[1][k] not in stop_words + punctuations) and ((sourceNeighborhood[0][k], targetNeighborhood[0][l]) in alignments or (wordRelatedness(sourceNeighborhood[1][k], 'none', targetNeighborhood[1][l], 'none')>=ppdbSim)):
...
#def align(sentence1, sentence2):
    ''' This function written by Md. Sultan, U Colorado
     checks for similar words in a pair of given sentences.
     Index [0] of the returned array reveals the pair of 
     similar words. The words can also be contexually similar.'''
...
#    sentence2ParseResult = parseText(sentence2)    
    ...
    #print [(sentence1LemmasAndPosTags[item[0]-1][2],
    #        sentence2LemmasAndPosTags[item[1]-1][2]) \
    #            for item in myWordAlignments[0] \
    #                if sentence1LemmasAndPosTags[item[0]-1][2].lower() not in \
    #                        stop_words+punctuations]

    #if newParseResults:
    #    pickle.dump(parse_results, open('sentence-parse-results.pickle', 'wb'))
...
#
#sentence1 = "They are going for food."
#sentence2 = "They are going to eat something."
#
#print align(sentence1, sentence2,parseText(sentence1),parseText(sentence2))[0]
